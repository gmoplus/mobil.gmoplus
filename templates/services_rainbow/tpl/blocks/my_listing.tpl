	...
	<div class="nav">
		...
		<div class="navigation">
			<ul>
				...
				{* Quote System - Show quotes for listing *}
				{if $listing.quote_count > 0}
					<li class="nav-icon">
						<a class="quotes" href="#" onclick="viewListingQuotes({$listing.ID}); return false;" title="Tekliflerinizi görüntüleyin">
							<span>Teklifler ({$listing.quote_count})</span>
							{if $listing.new_quote_count > 0}
								<span class="badge" style="background: #dc3545; color: white; border-radius: 10px; padding: 2px 6px; font-size: 11px; margin-left: 5px;">{$listing.new_quote_count}</span>
							{/if}
						</a>
					</li>
				{/if}
