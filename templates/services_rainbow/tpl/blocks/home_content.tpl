...
<section class="point1 m-auto position-relative horizontal-search{if $plugins.search_by_distance} sbd-exists{/if}">
    ...
    <form class="pl-4 pr-4 pt-4 pb-4 w-100" accesskey="{pageUrl key='search'}#keyword_tab" method="post" action="{if $cdt_count == 0}{pageUrl key='search'}{else}{$rlBase}{if $config.mod_rewrite}{$spage_path}/{$search_results_url}.html{else}?page={$spage_path}&{$search_results_url}{/if}{/if}">
