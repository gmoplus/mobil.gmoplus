...
class rlSearch
{
    ...
    public function search($data = false, $type = false, $start = 0, $limit = false)
    {
        ...
        foreach ($form as $fVal) {
            if (!is_array($fVal) || !isset($fVal['Condition']) || !isset($fVal['Key'])) {
                continue;
            }
        ...
        foreach ($form as $fKey => $fVal) {
            if (!is_array($fVal) || !isset($fVal['Type'])) {
                continue;
            }
...
//        }
        ...
        } elseif (isset($form[$data['sort_by']]) && is_array($form[$data['sort_by']])) {
            switch ($form[$data['sort_by']]['Type']) {
                case 'mixed':
                    if (isset($form[$data['sort_by']]['Condition']) && $GLOBALS['conversion_rates'][$form[$data['sort_by']]['Condition']]) {
                ...
                case 'unit':
                    ...
                    break;
                case 'select':
                    if (isset($form[$data['sort_by']]['Key']) && $form[$data['sort_by']]['Key'] == 'Category_ID') {
                        $sql .= ", `T3`.`Key` " . strtoupper($data['sort_type']) . " ";
                    } elseif (isset($form[$data['sort_by']]['Key']) && $form[$data['sort_by']]['Key'] == 'Listing_type') {
        ...
        $listings = is_array($listings) ? $GLOBALS['rlLang']->replaceLangKeys($listings, 'categories', 'name') : array();

        // Check if listings is an array before using foreach
        if (is_array($listings)) {
            foreach ($listings as &$listing) {
                ...
                if ($data['myads_controller'] && empty($listing['Featured'])) {
                ...
                if (is_array($fields)) {
                    foreach ($fields as &$field) {
                        ...
                        $field['value'] = $rlCommon->adaptValue(
                            $field,
                            isset($listing[$field['Key']]) ? $listing[$field['Key']] : null,
                            'listing',
                            $listing['ID'],
                            true,
                            false,
