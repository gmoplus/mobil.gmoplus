...
if (defined('IS_LOGIN')) {
    ...
    /* browse mode */
        ...
        // Load quote system and get quote counts for each listing
        if ($listings && file_exists(RL_INC . 'classes/rlQuoteSystem.class.php')) {
            require_once RL_INC . 'classes/rlQuoteSystem.class.php';
            $quoteSystem = rlQuoteSystem::getInstance();
            
            foreach ($listings as &$listing) {
                $quotes = $quoteSystem->getQuotesForListing($listing['ID']);
                $listing['quote_count'] = count($quotes);
                $listing['new_quote_count'] = 0;
                
                if ($quotes) {
                    foreach ($quotes as $quote) {
                        if ($quote['status'] == 'new') {
                            $listing['new_quote_count']++;
                        }
                    }
                }
            }
