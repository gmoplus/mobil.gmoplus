...
class Category
{
    ...
    public static function buildForm($category, &$listing_type, &$fields_list = array())
    {
        ...
        $form = array(); // Initialize form as empty array

        // get from cache
        if ($config['cache']) {
            $form = $rlCache->get('cache_submit_forms', $category_id, $listing_type, $parent_ids);

            // Check if form exists and is array
            if (is_array($form)) {
                foreach ($form as $group_key => &$group) {
                    // Check if Fields exists and is an array, if not skip this group
                ...
                foreach ($form as $key => &$value) {
                    ...
                    if (isset($value['Fields']) && $value['Fields']) {
                        ...
                        if (empty($fields)) {
                            ...
                            // assign name - check if Fields exists and is an array first
                        ...
                        $form[$key]['Fields'] = array(); // Set as empty array instead of false
                    }
                }
            }
        }
        ...
        // Final cleanup - ensure all Fields are arrays and remove any problematic data
        if (is_array($form)) {
            foreach ($form as $group_key => &$group) {
                ...
                // If Fields doesn't exist or is not an array, set it as empty array
                if (!isset($group['Fields']) || !is_array($group['Fields'])) {
                    $group['Fields'] = array();
                }
                
                // Clean up any non-array Fields
                if (is_string($group['Fields']) || is_numeric($group['Fields'])) {
                    $group['Fields'] = array();
                }
            }
        } else {
            // If form is not an array, return empty array
            $form = array();
        }
